...
// const Shopbybrandfilter = ({ navigation, route }) => {
//   const { brandname, catName } = route?.params;
//   const [brandsData, setBrandsData] = useState([]);
//   const [loading, setLoading] = useState(false);
...
const Shopbybrandfilter = ({ navigation, route }) => {
  ...
  useEffect(() => {
    if (catList?.length > 0) {
      const defaultTab =
        catList.find(c => c.category_name === catName) || catList[0];
  ...
  useEffect(() => {
    if (!activeTab?.category_name) return;
  ...
  return (
    <View style={styles.container}>
      <Header title={brandname} navigation={navigation} showBack showSearch />
        ...
        {catList.map(cat => (
          ...
          >
            <Text
              style={[
                styles.tabText,
                activeTab?.id === cat.id && styles.activeTabText,
              ]}
            >
              {cat.category_name}
            </Text>
